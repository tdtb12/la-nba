...
export default function AddExpense() {
    ...
    return (
        <div className="relative flex h-full min-h-screen w-full max-w-[430px] mx-auto flex-col bg-background-dark overflow-x-hidden shadow-2xl font-display text-white">
            <header className="sticky top-0 z-20 flex items-center bg-background-dark/80 backdrop-blur-md px-4 pt-4 pb-4 justify-between border-b border-white/10">
                <button onClick={() => navigate(-1)} className="text-primary text-base font-medium">取消</button>
                <h1 className="text-white text-[17px] font-bold">新増分帳</h1>
            ...
            <main className="flex-1 overflow-y-auto custom-scrollbar px-4 pt-6 pb-32">
                <div className="bg-card-dark rounded-xl p-5 mb-6 shadow-sm border border-white/5">
                    <div className="flex flex-col gap-6">
                        <div className="flex flex-col items-center justify-center py-4">
                            <div className="flex items-center gap-2 mb-2">
                                <p className="text-slate-400 text-[10px] font-bold uppercase tracking-widest">總金額</p>
                                <div className="flex bg-black/20 p-0.5 rounded-full">
                                    <button onClick={() => setCurrency("USD")} className={`px-2 py-0.5 rounded-full text-[9px] font-bold transition-all ${currency === "USD" ? "bg-slate-700 text-white shadow-sm" : "text-slate-500"}`}>USD</button>
                                    <button onClick={() => setCurrency("TWD")} className={`px-2 py-0.5 rounded-full text-[9px] font-bold transition-all ${currency === "TWD" ? "bg-slate-700 text-white shadow-sm" : "text-slate-500"}`}>TWD</button>
                                </div>
                            </div>
                            <div className="flex items-center gap-1">
                                ...
                                <input
                                    ...
                                    className="bg-transparent border-none text-white text-5xl font-bold p-0 focus:ring-0 w-48 text-center placeholder:text-slate-700 placeholder:font-bold"
                                    placeholder="0.00"
                                    type="number"
                                    step="0.01"
                                />
                        ...
                        <div className="flex flex-col">
                            <label className="text-slate-400 text-[10px] font-bold uppercase tracking-widest mb-2">項目名稱</label>
                            <div className="flex items-center gap-3">
                                <span className="material-symbols-outlined text-primary text-xl">receipt_long</span>
                                <input
                                    value={item}
                                    onChange={(e) => setItem(e.target.value)}
                                    className="bg-transparent border-none text-white text-lg font-medium p-0 focus:ring-0 flex-1 placeholder:text-slate-600"
                                    placeholder="例如: 晚餐"
                                    type="text"
                                />
                            </div>
                ...
                <div className="bg-card-dark rounded-xl p-5 mb-6 shadow-sm border border-white/5">
                    ...
                    <div className="mt-6 p-1 bg-black/20 rounded-lg flex">
                        <button className="flex-1 py-2 px-4 rounded-md text-xs font-bold transition-all bg-card-dark text-white shadow-sm">平分</button>
                        <button className="flex-1 py-2 px-4 rounded-md text-xs font-bold transition-all text-slate-500">自訂金額</button>
                    </div>
                </div>
                ...
                <div className="bg-card-dark rounded-xl p-5 shadow-sm border border-white/5">
                    <div className="flex items-center justify-between">
                        <div className="flex flex-col">
                            <p className="text-slate-400 text-[10px] font-bold uppercase tracking-wider">預估每人</p>
                            <p className="text-white text-2xl font-bold">
                                {currency === 'USD' ? '$' : 'NT$'}
                                {amount && (selectedFriends.length + 1) > 0 ? (parseFloat(amount) / (selectedFriends.length + 1)).toFixed(2) : "0.00"}
                            </p>
                        </div>
                        <div className="bg-teal-500/10 text-teal-500 px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-tight">
            ...
            <div className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-[430px] p-6 bg-gradient-to-t from-background-dark via-background-dark to-transparent pt-10 z-30">
                <button onClick={handleSave} disabled={loading} className="w-full bg-primary hover:bg-primary/90 text-background-dark font-bold py-4 rounded-xl shadow-lg shadow-primary/20 transition-all flex items-center justify-center gap-2 active:scale-95 disabled:opacity-50">
